...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m0(duration) => "\t\tduration: ${duration}";

  static m1(name, ns, data) => "${name}\nNamespace: ${ns}\nData: ${data}";

  static m2(arg) => "Runtime:\t\t ${arg}";

  static m3(name, kind, scope, shortNames) => "${name}\n\nKind: ${kind}\nScope: ${scope}\nshortNames: ${shortNames}";

  static m4(name, ns, ready, upToDate, available) => "${name}\nNamespace: ${ns}\nReady: ${ready}\nUp to date: ${upToDate}\nAvailable: ${available}\n";

  static m5(error) => "delete failed, error: ${error}";

  static m6(name) => "${name} deleted";

  static m7(name, ns, ready, upToDate, available) => "${name}\nNamespace: ${ns}\nReady: ${ready}\nUp to date: ${upToDate}\nAvailable: ${available}";

  static m8(name, ns, endpoints) => "${name}\nNamespace: ${ns}\nEndpoints: ${endpoints}";

  static m9(namespace, name, type, reason, kind, objName, lastTimestamp, message) => "${namespace} / ${name}\n\nType: ${type}\nReason: ${reason}\nObject: ${kind}/${objName}\nLast Seen: ${lastTimestamp}\n\nMessage: ${message}\n";

  static m10(path) => "Exported to ${path}";

  static m11(arg) => "External-IP:\t\t ${arg}";

  static m12(name, ns, className, hosts, address, ports) => "${name}\nNamespace: ${ns}\nClass: ${className}\nHosts: ${hosts}\nAddress: ${address}\nPorts: ${ports}";

  static m13(arg) => "Internal-IP:\t\t ${arg}";

  static m14(number) => " (${number} items)";

  static m15(n) => "last ${n} warnings";

  static m16(error) => "load metrics error: ${error}";

  static m17(n) => "${n} s";

  static m18(arg) => "Architecture\t\t: ${arg}";

  static m19(os, arg) => "Kernel:\t\t ${os}/${arg}";

  static m20(arg) => "Kernel:\t\t ${arg}";

  static m21(arg) => "Roles:\t\t ${arg}";

  static m22(arg) => "Version:\t\t ${arg}";

  static m23(name, namespace, ready, status, restarts, containers, cpu, memory) => "${name}\n\nNamespace: ${namespace}\nReady: ${ready}\nStatus: ${status}\nRestarts: ${restarts}\nContainers: ${containers}\nCPU: ${cpu}\nMemory: ${memory}";

  static m24(name, capacity, accessModes, reclaimPolicy, status, claim, storageClass, reason) => "${name}\nCapacity: ${capacity}\nAccess Modes: ${accessModes}\nReclaim Policy: ${reclaimPolicy}\nStatus: ${status}\nClaim: ${claim}\nStorage Class: ${storageClass}\nReason: ${reason}\n";

  static m25(name, ns, status, volume, capacity, accessModes, storageClass) => "${name}\nNamespace: ${ns}\nStatus: ${status}\nVolume: ${volume}\nCapacity: ${capacity}\nAccess Modes: ${accessModes}\nStorage Class: ${storageClass}";

  static m26(name, ns, revision, appVer, updated, status, chart) => "${name}\nNamespace: ${ns}\nRevision: ${revision}\nApp Version: ${appVer}\nUpdated: ${updated}\nStatus: ${status}\nChart: ${chart}";

  static m27(name, ns, current, ready, available) => "${name}\nNamespace: ${ns}\nCurrent: ${current}\nReady: ${ready}\nAvailable: ${available}\n";

  static m28(error) => "scale failed, error: ${error}";

  static m29(N) => "Scale to ${N} replica(s)";

  static m30(name, ns, type, data) => "${name}\nNamespace: ${ns}\nType: ${type}\nData: ${data}";

  static m31(name, ns, secrets) => "${name}\nNamespace: ${ns}\nSecret: ${secrets}";

  static m32(name, ns, type, clusterIP, externalIP, ports) => "${name}\nNamespace: ${ns}\nType: ${type}\nCluster IP: ${clusterIP}\nExternal IP: ${externalIP}\nPorts: ${ports}";

  static m33(name, ns, ready, upToDate, available) => "${name}\nNamespace: ${ns}\nReady: ${ready}\nUp to date: ${upToDate}\nAvailable: ${available}\n";

  static m34(name, provisioner, reclaimPolicy, mountOptions, volumeBindingMode, allowVolumeExpansion) => "${name}\nProvisioner: ${provisioner}\nReclaim Policy: ${reclaimPolicy}\nVolume Binding Mode: ${volumeBindingMode}\nAllow Volume Expansion: ${allowVolumeExpansion}\nMountOptions: ${mountOptions}";

  static m35(date) => "Sponsor expired: \$${date}";

  static m36(error) => "Restore Purchases Failed, ERROR: ${error}";

  static m37(number) => "${number} terminals opened";

  static m38(number) => "Totals: ${number}";

  static m39(type, name) => "will delete ${type} ${name}";
  ...
  static _notInlinedMessages(_) => <String, Function> {
    ...
    "blockOwnerDeletion" : MessageLookupByLibrary.simpleMessage("blockOwnerDeletion"),
    ...
    "debug_flushdb_desc" : MessageLookupByLibrary.simpleMessage("will flush all data at database"),
    "debug_flushdb_done" : MessageLookupByLibrary.simpleMessage("database flushed"),
    "delete" : MessageLookupByLibrary.simpleMessage("Delete"),
    "delete_failed" : m5,
    "delete_ok" : MessageLookupByLibrary.simpleMessage("delete resource success."),
    "delete_resource" : MessageLookupByLibrary.simpleMessage("Are you sure delete the resource?"),
    "deleted" : m6,
    "deployment_text" : m7,
    "deployments" : MessageLookupByLibrary.simpleMessage("Deployments"),
    "discovery_and_lb" : MessageLookupByLibrary.simpleMessage("Discovery and Load Balancing"),
    "dnsPolicy" : MessageLookupByLibrary.simpleMessage("DNS Policy"),
    "documents" : MessageLookupByLibrary.simpleMessage("Documents"),
    "edit" : MessageLookupByLibrary.simpleMessage("Edit"),
    "empty" : MessageLookupByLibrary.simpleMessage("empty"),
    "empyt_context" : MessageLookupByLibrary.simpleMessage("can not get cluster kubeconfig, contexts maybe empty"),
    "endpoint_text" : m8,
    "endpoints" : MessageLookupByLibrary.simpleMessage("Endpoints"),
    "ephemeral_containers" : MessageLookupByLibrary.simpleMessage("Ephemeral Containers"),
    ...
    "imagePullPolicy" : MessageLookupByLibrary.simpleMessage("Image Pull Policy"),
    "imagePullSecrets" : MessageLookupByLibrary.simpleMessage("Image Pull Secrets"),
    ...
    "load_balancer_ip" : MessageLookupByLibrary.simpleMessage("Load Balancer IP"),
    ...
    "manual_load_kubeconfig" : MessageLookupByLibrary.simpleMessage("Load kubeconfig file"),
    ...
    "no_current_cluster" : MessageLookupByLibrary.simpleMessage("no current cluster"),
    "node_arch" : m18,
    "node_kernel" : m19,
    "node_os_image" : m20,
    "node_roles" : m21,
    ...
    "nodes_desc" : MessageLookupByLibrary.simpleMessage("A node may be a virtual or physical machine."),
    ...
    "select_clusters" : MessageLookupByLibrary.simpleMessage("Select cluster(s)"),
    ...
    "session_affinity" : MessageLookupByLibrary.simpleMessage("Session Affinity"),
    ...
    "subscriptions_purchased" : MessageLookupByLibrary.simpleMessage("Purchased"),
    "subscriptions_restorePurchases_failed" : m36,
    "subscriptions_restore_purchases" : MessageLookupByLibrary.simpleMessage("Restore Purchases"),
    "subscriptions_restore_success" : MessageLookupByLibrary.simpleMessage("Resotre success."),
