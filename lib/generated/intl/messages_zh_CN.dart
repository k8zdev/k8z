...
class MessageLookup extends MessageLookupByLibrary {
  ...
  static m10(path) => "已导出到 ${path}";

  static m11(arg) => "外部 IP:\t\t ${arg}";

  static m12(name, ns, className, hosts, address, ports) => "${name}\n名字空间: ${ns}\n类: ${className}\n主机: ${hosts}\n地址: ${address}\n端口: ${ports}";

  static m13(arg) => "内部 IP:\t\t ${arg}";

  static m14(number) => " (${number} 项)";

  static m15(n) => "最近 ${n} 警告";

  static m16(error) => "加载指标错误: ${error}";

  static m17(n) => "${n} 秒";

  static m18(arg) => "架构\t\t: ${arg}";

  static m19(os, arg) => "内核:\t\t ${os}/${arg}";

  static m20(arg) => "系统镜像:\t\t ${arg}";

  static m21(arg) => "角色:\t\t ${arg}";

  static m22(arg) => "版本:\t\t ${arg}";
  ...
  static m28(error) => "扩缩容失败, 错误: ${error}";

  static m29(N) => "缩放至 ${N} 副本";
  ...
  static m35(date) => "赞助过期时间: \$${date}";

  static m36(error) => "恢复购买失败, 错误: ${error}";

  static m37(number) => "已打开 ${number} 个终端";

  static m38(number) => "总计: ${number}";

  static m39(type, name) => "将要删除 ${type} ${name}";
  ...
  static _notInlinedMessages(_) => <String, Function> {
    ...
    "empyt_context" : MessageLookupByLibrary.simpleMessage("无法从 kubeconfig 读取集群信息, contexts 字段可能为空"),
    "endpoint_text" : m8,
    ...
    "manual_load_kubeconfig" : MessageLookupByLibrary.simpleMessage("加载 kubeconfig 文件"),
    ...
    "sponsor_desc" : MessageLookupByLibrary.simpleMessage("赞助我以便于我可以继续开发维护这款应用程序。"),
    "sponsorme" : MessageLookupByLibrary.simpleMessage("赞助我"),
    "start_debug" : MessageLookupByLibrary.simpleMessage("开始调试"),
    "start_debug_desc" : MessageLookupByLibrary.simpleMessage("开始调试将会在 Pod 中创建一个临时容器, 并将 stdin, stdout, stderr 附加到它。"),
    ...
    "subscriptions_iap_desc" : MessageLookupByLibrary.simpleMessage("如不取消, 订购将会自动续费。付款将在确认购买时向iTunes帐户收取。订阅自动续订, 除非自动续订在当前期限结束前至少24小时关闭。账户将在本期结束前的24小时内收取续费费用, 并确定续约费用。订购可由用户管理, 购买后可通过转到用户的帐户设置关闭自动续订。免费试用期的任何未使用部分（如果提供）在用户购买该出版物的订阅时将被没收。"),
    ...
    "subscriptions_restore_purchases" : MessageLookupByLibrary.simpleMessage("恢复购买"),
    "subscriptions_restore_success" : MessageLookupByLibrary.simpleMessage("恢复成功"),
